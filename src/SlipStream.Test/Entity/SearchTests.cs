...
namespace SlipStream.Entity.Test
{
    ...
    public class SearchTests : ServiceContextTestCaseBase
    {
        ...
        {
            var entityEntity = this.GetResource(Core.MetaEntityEntity.EntityName);
            var constraint = new object[][] { };
            dynamic ids = null;
            Assert.DoesNotThrow(() =>
            {
                ids = entityEntity.Search(constraint, null, 0, 0);
            });
        ...
        {
            var modelModel = this.GetResource(Core.MetaEntityEntity.EntityName);
        ...
        public void CanUseEqualOperator()
        {
            var modelModel = this.GetResource(Core.MetaEntityEntity.EntityName);
            var constraint = new object[][] {
                new object[] {  "name", "=", Core.MetaEntityEntity.EntityName }
            };
        ...
        {
            var modelModel = this.GetResource("core.meta_entity");

            var constraint = new object[][] { new object[] { "name", "like", "core.meta_entit%" } };
            var ids = modelModel.Search(constraint, null, 0, 0);
            Assert.AreEqual(1, ids.Length);
        ...
        public void CanUseInOperator()
        {
            var constraint = new object[][] {
                new object[] {
                    "name", "in",
                    new object[] { "core.meta_entity", "core.meta_field", "core.module" }
                }
            };
            var modelModel = this.GetResource("core.meta_entity");
        ...
        {
            dynamic modelModel = this.GetResource("core.meta_entity");
            dynamic allIds = modelModel.Search(null, null, 0, 0);

            var notinDomain = new object[][] {
                new object[] {
                    "name", "!in",
                    new object[] { "core.meta_entity", "core.meta_field" }
                }
            };

...
namespace SlipStream.Entity.Fields
{
    ...
    {
        private const string EntityName = "test.functional_field_entities";

        private dynamic PrepareTestData()
        {
            dynamic data = new ExpandoObject();
            dynamic model = this.GetResource(EntityName);
        ...
        {
            dynamic entity = this.GetResource(EntityName);
            long[] ids = (long[])entity.Search(null, null, 0, 0);
            var idsToDel = ids.Select(o => (object)o).ToArray();
            entity.Delete(idsToDel);
        }
        ...
        public void CanUseFunctionalManyToOneField()
        {
            ...
            dynamic userEntity = this.GetResource("core.user");
            dynamic entity = this.GetResource(EntityName);
            dynamic ids = userEntity.Search(constraints, null, 0, 0);

            dynamic data = PrepareTestData();

            dynamic records = entity.Read(new object[] { data.record1_id }, null);
        ...
        public void CanFunctionalFieldAsConstraint()
        {
            ...
            dynamic n = entity.Count(constraint);
            Assert.AreEqual(1, n);
        }
    }
}

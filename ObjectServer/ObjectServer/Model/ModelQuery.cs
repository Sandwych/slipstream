...
namespace ObjectServer.Model
{
    internal sealed class ModelQuery
    {
        ...
        public ModelQuery(IResourceScope ctx, IMetaModel model)
        {
            this.context = ctx;
            this.model = model;
        }

        public long[] Search(object[] domain, long offset = 0, long limit = 0)
        {
            if (domain == null)
            {
            ...
            using (var cmd = this.context.DatabaseProfile.DataContext.Connection.CreateCommand())
            {
                cmd.CommandText = sql;
